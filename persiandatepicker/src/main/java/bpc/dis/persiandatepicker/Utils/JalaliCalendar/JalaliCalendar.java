package bpc.dis.persiandatepicker.Utils.JalaliCalendar;
...
public class JalaliCalendar extends Calendar {
    ...
    private static final int MIN_VALUES[] = {BCE, 1, FARVARDIN, 1, 0, 1, 1, SATURDAY, 1, AM, 0, 0, 0, 0, 0, -13 * ONE_HOUR, 0};
    private static final int LEAST_MAX_VALUES[] = {CE, 292269054, ESFAND, 52, 4, 28, 365, FRIDAY, 4, PM, 11, 23, 59, 59, 999, 14 * ONE_HOUR, 20 * ONE_MINUTE};
    private static final int MAX_VALUES[] = {CE, 292278994, ESFAND, 53, 6, 31, 366, FRIDAY, 6, PM, 11, 23, 59, 59, 999, 14 * ONE_HOUR, 2 * ONE_HOUR};
    ...
    private JalaliCalendar(TimeZone zone, Locale aLocale) {
        super(zone, aLocale);
        timeZone = zone;
    ...
    private JalaliCalendar(int year, int month, int dayOfMonth, int hourOfDay, int minute, int second, int millis) {
        super();
        this.set(YEAR, year);
    ...
    private static YearMonthDate gregorianToJalali(YearMonthDate gregorian) {
        ...
        gregorianDayNo = 365 * gregorian.getYear() + (int) Math.floor((gregorian.getYear() + 3) / 4) - (int) Math.floor((gregorian.getYear() + 99) / 100) + (int) Math.floor((gregorian.getYear() + 399) / 400);
    ...
    private static YearMonthDate jalaliToGregorian(YearMonthDate jalali) {
    ...
    private static int weekOfYear(int dayOfYear, int year) {
    ...
    private static int dayOfWeek(YearMonthDate yearMonthDate) {
    ...
    protected void computeTime() {
        if (!isTimeSet && !isTimeSeted) {
            ...
            cal.set(yearMonthDate.getYear(), yearMonthDate.getMonth(), yearMonthDate.getDate(), internalGet(HOUR_OF_DAY), internalGet(MINUTE), internalGet(SECOND));
            time = cal.getTimeInMillis();
    ...
    public void set(int field, int value) {
        switch (field) {
            ...
            case ZONE_OFFSET:
            case DST_OFFSET: {
                if (isSet(YEAR) && isSet(MONTH) && isSet(DATE) && isSet(HOUR) && isSet(HOUR_OF_DAY) && isSet(MINUTE) && isSet(SECOND) && isSet(MILLISECOND)) {
                    cal = new GregorianCalendar();
    ...
    public static class YearMonthDate {
        ...
        private int month;
        private int date;
        private YearMonthDate(int year, int month, int date) {
            this.year = year;
            this.month = month;
            this.date = date;
        }
